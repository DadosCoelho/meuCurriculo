...
function setupSavePDFButton() {
    ...
    if (saveButton) {
        saveButton.addEventListener('click', () => {
            // Obtém o nome da pessoa do elemento com o ID 'nome-header'
            const nomePessoa = document.getElementById('nome-header').textContent;

            // Remove caracteres especiais e espaços do nome para criar um nome de arquivo válido
            const nomeArquivo = nomePessoa.replace(/[^a-zA-Z0-9]/g, '_').toLowerCase() + '.pdf';

            // Cria um link temporário
            const link = document.createElement('a');
            link.href = 'meu_curriculo.pdf'; // Define o caminho para o arquivo PDF existente
            link.download = nomeArquivo; // Define o nome do arquivo para o download com o nome da pessoa
...
function updateCurriculoUI(data) {
    ...
    updateHTML('github', `<a href="${data.contato.github}" >${data.contato.github.split('/').pop()}</a>`);
    ...
    featuredRepos.forEach((repo, index) => {
        setTimeout(() => {
            ...
            const projectNameHTML = hasHomepage
                ? `<a href="${repo.homepage}" >${repo.name} <span class="link-icon">🔗</span></a>`
                : `${repo.name}`;
                ...
                <div class="card-inner">
                    ...
                    <div class="card-body">
                        ...
                        <div class="card-actions">
                            <a href="${repo.html_url}"  class="card-button">
